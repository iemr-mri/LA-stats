"""Mixed-effects analysis of longitudinal LA volume, by age and sex.

For each volume measure (max, min, stroke): a random-intercept linear mixed
model with categorical age x sex fixed effects over the Aging cohort, the
likelihood-ratio test for the age x sex interaction, estimated marginal means
and pairwise age contrasts per sex, plus residual diagnostics. Ends with a
sensitivity refit that pools in the healthy MI/AB Baseline scans. Run from the
project root:
...
from src.plotting import SEX_ORDER, plot_residual_diagnostics  # noqa: E402
...
def report_outcome(measurements, value_column, label):
    """Fit, test and summarise one volume outcome; save residual diagnostics."""
    print(f"\n{'=' * 70}\n{label}  ({value_column})\n{'=' * 70}")
